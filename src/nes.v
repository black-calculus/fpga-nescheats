...
//           output [1:0] dbgctr
  ...
  //wire [7:0] ppu_dbus_corrupt = dbus[6:0] + 7'b0000001; // testing some address manipulation
  //wire [3:0] ppu_addr_corrupt = addr[2:0] + 3'b010;

  // Mucking about with color information
  wire [5:0] color_corrupted;
  PPU ppu(clk, ce, reset, color_corrupted, dbus, ppu_dout, addr[2:0], // PPU ppu(clk, ce, reset, color, dbus, ppu_dout, addr[2:0],
          ppu_cs && mr_ppu, ppu_cs && mw_ppu,
          nmi,
          chr_read, chr_write, chr_addr, chr_to_ppu, chr_from_ppu,
          scanline, cycle, mapper_ppu_flags);
  wire [6:0] color_corruption = color_corrupted + 6'b010001;
  assign color = color_corruption[5:0];
